...
\begin{proof} WLOG take $-a = b = \pi$\\
    ...
    \footnotetext{Meaning they can be written as a power series}
...
We want to make the assumptions of theorems as less as possible. In this quest, a new class of \emph{well-behaving} functions should be defined: \emph{functions of bounded variations.} \footnotemark{*?*}

\footnotetext{to be asked!}
...
\begin{remark}\label{alternative riemann-lebesque lemma}
    If $f$ is integrable on $[-\pi, \pi]$ and $\alpha \in \RR$, then \footnotemark{*?*}


     \footnotetext{What to do when k is real number?, should I state k is natural?}
...
\footnotetext{The body of the book is taken from \cite{Measure}}
...
\begin{proof}
    ...
    form the branch \emph{series of function} gives us the necessary argument to claim that equality holds for every bounded $E \subset \RR$.\footnotemark{*?*}
...
\footnotetext{To be asked! Probably wrong but why?}
